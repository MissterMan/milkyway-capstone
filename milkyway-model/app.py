...
donor_df = pd.read_csv('donor_breastfeed_500.csv')
...
def recommend():
    ...
    # Mengambil data lokasi pada file CSV yang memiliki 5000 baris data
    donor_addresses = donor_df['Location'].to_list()
    donor_coordinates = []
    
    # Looping seluruh lokasi yang ada di dalam file CSV
    # Google Maps Geocoding API akan dipanggil sebanyak jumlah data yang ada di file CSV
    ...
    top_k_similar_donors_data = top_k_similar_donors.to_dict(orient='records')

    return jsonify(top_k_similar_donors_data, ) 
